...
__global__ void eval_many_at_point_first_pass(m31 **g_coeffs, qm31 *temp, qm31 *factors, int coeffs_size, int factors_size,
    ...
    int global_thread_index = blockIdx.x * blockDim.x + idx;
    int poly_index = 2 * global_thread_index / coeffs_size;
    s_coeffs[idx] = g_coeffs[poly_index][2 * global_thread_index];
    s_coeffs[idx + blockDim.x] = g_coeffs[poly_index][2 * global_thread_index + blockDim.x];
    __syncthreads();
...
__global__
void copy_result_for_polynomial(qm31 **result, qm31 *temp, int temp_offset, int number_of_polynomials) {
    int global_thread_index = blockIdx.x * blockDim.x + threadIdx.x;

    if (global_thread_index < number_of_polynomials) {
        result[global_thread_index][0] = temp[temp_offset + global_thread_index];
    }
}
...
void eval_polys_at_point(
    ...
    while (size > log_number_of_polynomials) {
        size = (size + 511) / 512;
        temp_memory_size += size;
    }
    ...
    // First pass
    int block_dim = 256;
    int num_blocks = ((total_number_of_coeffs >> 1) + block_dim - 1) / block_dim;
    int shared_memory_bytes = 512 * 4 + 512 * 8;
    int output_offset = temp_memory_size - num_blocks;
    ...
    copy_result_for_polynomial<<<number_of_polynomials, block_dim>>>(
        result, temp, temp_memory_size - number_of_polynomials, number_of_polynomials
    );
